...
Ogni articolo ha frontmatter YAML: `title`, `description`, `date`, `tags`, `lang`, `translationKey`, `headerImage` (opzionale), `draft` (opzionale — i draft non compaiono in home né nel feed RSS, ma la pagina esiste), `hidden` (opzionale — l'articolo può stare nel repo ma **in produzione non viene generato affatto**: niente pagina, niente lista, niente RSS. In `npm run dev` invece è visibile, con badge "nascosto", per l'anteprima. Per abilitarlo, rimuovi `hidden: true` e fai push).
